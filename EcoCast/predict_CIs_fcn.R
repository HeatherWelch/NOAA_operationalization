...
predCIs_master <- function(get_date,envdir,moddir,outdir,path,final_path_list,staticdir){
  ...
  predsurfsDF[missing1]=NA
   ...
   ############ 4. rename colunms to reflect every possible model variable naming convention
    ...
    ############ 5. load base grid
    baseGrid <- readRDS(paste(staticdir,'/baseGrid_full.rds',sep=""))  # change for each user, or make global
